...
import enigma
import csv
from plugin import cfg, hdr, screenwidth, regionlist, schedulelist, regionbouquets
...
class SlykEpg7Day_Main(ConfigListScreen, Screen):
    ...
    def __init__(self, session, runtype):
        ...
        if self.runtype == "manual":
            self.skin = skin
            self.setup_title = _('Slyk EPG 7 Day Downloader')
    ...
    def initConfig(self):
        self.cfg_region =       getConfigListEntry(_('Primary Sky Region'), cfg.region, _('Select the main region for your EPG.'))
        self.cfg_enabled =      getConfigListEntry(_('Automatic import Slyk EPG'), cfg.enabled, _('Select to automatically download EPG data daily.'))
        #self.cfg_schedule =     getConfigListEntry(_('Day(s) of the week to download'), cfg.schedule, _('Select the day(s) of the week in which you would like to download EPG data.'))
        self.cfg_wakeup =       getConfigListEntry(_('Automatic start time'), cfg.wakeup, _('Select the daily download time.\nSelect a random unique time and not on the hour like 8:00am.\nHelp prevent server overload.'))
        self.cfg_epgDescDays =  getConfigListEntry(_('Load EPG descriptions up to X days'), cfg.epgDescDays, _('Select the maximum number of days you would like to download EPG data for. 1 - 7 days.'))
        self.cfg_timeshift =    getConfigListEntry(_('EPG timeshift offset'), cfg.timeshift, _('Adjust the timeshift according to UK Daylight Saving Time and/or your local timezone.\nUK: Summer = 1, Winter = 0'))
        self.cfg_ukcable =      getConfigListEntry(_('Try populate UK Cable/Virgin EPG?'), cfg.ukcable, _("Select this option if you want to also populate UK cable with Sky's EPG data."))
        self.cfg_lamedb =       getConfigListEntry(_('Use provided Sat 28.2e lamedb file'), cfg.lamedb, _('Select this option if you do not have 28.2e satellite feeds. i.e UK Cable with no satellite/None UK/IPTV only.'))
        self.cfg_rytecIDs =     getConfigListEntry(_('XMLTV Channel ID references'), cfg.rytec, _("Select whether to try and match Rytec's original EPG IDs or whether just to create unique IDs for the XMLTV files."))
        self.cfg_compress =       getConfigListEntry(_('Compress XMLTV programme data (slower)'), cfg.compress, _('Select to compress the XMLTV files if harddrive space is limited.'))
    ...
    def loadLamedbFile(self):
        ...
        if os.path.isfile(ignorefile):
            with open(ignorefile) as f:
                try:
                    ...
                    print '\n******** check ignore-lisr.txt file with https://jsonlint.com ********'
                ...
                with open(lamedb, 'rb') as f:
                    for line in f:
                        ...
                        if transponders:
                            ...
                            # check for cable
                            if cfg.ukcable.value == True:
                                if dataline.startswith('c'):
                          
                                    dvbline = dvbline.split(':')
                                    namespace = dvbline[0]
                                    
                                    if namespace not in cableTransponders:
                                        cableTransponders.append(namespace)
                        ...
                        if services:
                            ...
                            if any(x in DBVStreamData for x in cableTransponders):
                                if not ChannelName.isdigit() and ChannelName != "" and ChannelName not in ignorelist and "XXX" not in ChannelName and '2:0:0' not in DBVStreamData and "0x" not in ChannelName:
                                    lamedbcable.append([DBVStreamData, ChannelName, piconChannelName])
        ...
        c = list()
        for sublist in lamedbcable:
            if sublist not in c:
                c.append(sublist)

        lamedbcable = c
        lamedbcable.sort(key=lambda x: x[1])
        del c
        
        self.lamedb = lamedbsat + lamedbcable 
        ...
        with open('/etc/enigma2/SlykEpg7day/output/lamddbcable.csv', 'w') as f:   
            writer = csv.writer(f, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
            writer.writerow(['DBVStreamData', 'Original Name', 'Picon Name'])
            for item in lamedbcable:
                writer.writerow([item[0], item[1], item[2]])
        ...
        print "SlykEPG: List of cable namespaces: %s" % (cableTransponders)
        print "SlykEPG: Corresponding Lamedb cable entries: %s" % (len(lamedbcable))
    ...
    def makeEpgID(self):
        ...
        for channel in self.channels_all:
            ...
            #ITV Regions
            if   channel['sid'] == 6089: channel['t'] = "ITV Anglia East"
            elif channel['sid'] == 1045: channel['t'] = "ITV Anglia HD"
            elif channel['sid'] == 6128: channel['t'] = "ITV +1 Anglia"
            
            elif channel['sid'] == 6110: channel['t'] = "ITV Border"
            elif channel['sid'] == 1061: channel['t'] = "ITV Border England HD"
            elif channel['sid'] == 1020: channel['t'] = "ITV Border Scotland"
            
            elif channel['sid'] == 6381: channel['t'] = "ITV Anglia West"
            
            elif channel['sid'] == 6300: channel['t'] = "ITV Central"
            elif channel['sid'] == 6503: channel['t'] = "ITV Central West HD"
            elif channel['sid'] == 6145: channel['t'] = "ITV +1 Central"
            
            elif channel['sid'] == 6011: channel['t'] = "ITV Central East"
            
            elif channel['sid'] == 6200: channel['t'] = "ITV Channel Isles"
            
            elif channel['sid'] == 6130: channel['t'] = "ITV Granada"
            elif channel['sid'] == 6505: channel['t'] = "ITV Granada HD"
            elif channel['sid'] == 6355: channel['t'] = "ITV +1 Granada"
            
            elif channel['sid'] == 6143: channel['t'] = "ITV Meridian North"
            elif channel['sid'] == 6142: channel['t'] = "ITV Meridian East"
            
            elif channel['sid'] == 6390: channel['t'] = "ITV Tyne Tees"
            elif channel['sid'] == 1043: channel['t'] = "ITV Tyne Tees HD"
            elif channel['sid'] == 6126: channel['t'] = "ITV +1 Tyne Tees"
            
            elif channel['sid'] == 6020: channel['t'] = "ITV Wales"
            elif channel['sid'] == 6501: channel['t'] = "ITV Wales HD"
            elif channel['sid'] == 6012: channel['t'] = "ITV +1 Wales"
            
            elif channel['sid'] == 6030: channel['t'] = "ITV West"
            elif channel['sid'] == 1063: channel['t'] = "ITV Westcountry West HD"
            elif channel['sid'] == 6127: channel['t'] = "ITV +1 West"
            
            elif channel['sid'] == 6040: channel['t'] = "ITV West Country"
            elif channel['sid'] == 1062: channel['t'] = "ITV Westcountry SW HD"
            elif channel['sid'] == 6125: channel['t'] = "ITV +1 West Country"
            
            elif channel['sid'] == 6140: channel['t'] = "ITV Meridian South"
            elif channel['sid'] == 6502: channel['t'] = "ITV Meridian South HD"
            elif channel['sid'] == 6365: channel['t'] = "ITV +1 Meridian"
        
            elif channel['sid'] == 6161: channel['t'] = "ITV Yorkshire East"
            elif channel['sid'] == 1044: channel['t'] = "ITV Yorkshire HD"
            elif channel['sid'] == 6065: channel['t'] = "ITV +1 Yorkshire"
            
            elif channel['sid'] == 6160: channel['t'] = "ITV Yorkshire West"
            
            elif channel['sid'] == 6000: channel['t'] = "ITV London"
            elif channel['sid'] == 6504: channel['t'] = "ITV London HD"
            elif channel['sid'] == 6155: channel['t'] = "ITV +1 London"
            
            #STV Regions
            elif channel['sid'] == 6325: channel['t'] = "STV Dundee"
            elif channel['sid'] == 1167: channel['t'] = "STV Dundee HD"
            
            elif channel['sid'] == 6210: channel['t'] = "STV Grampian"
            elif channel['sid'] == 1168: channel['t'] = "STV Grampian HD"
            
            elif channel['sid'] == 6371: channel['t'] = "STV Scotland East"
            elif channel['sid'] == 1170: channel['t'] = "STV Scotland East HD"
            
            elif channel['sid'] == 6220: channel['t'] = "STV Scotland West"
            elif channel['sid'] == 4055: channel['t'] = "STV Scotland West HD"
            
            """
            elif channel['sid'] == 4007: channel['t'] = "SkySpMainEvHD (ROI)"
            elif channel['sid'] == 1309: channel['t'] = "SkySpMainEv (ROI)"
            
            elif channel['sid'] == 1010: channel['t'] = "SkySp PL HD (ROI)"
            elif channel['sid'] == 1312: channel['t'] = "SkySp PL (ROI)"
            """
    ...
    def addLamedbRefToChannelsJson(self):
        ...
        replacements = []
        replacementfile = '/etc/enigma2/SlykEpg7day/virgin-replacements.txt'
        if os.path.isfile(replacementfile) and os.stat(replacementfile).st_size > 0:
            with open(replacementfile) as f:
                try:
                    replacements = json.load(f)
                except ValueError as e:
                    print str(e) + '\n******** broken virgin-replacements.txt file ***********'
                    print '\n******** check /etc/enigma2/SlykEpg7day/virgin-replacements.txt file with https://jsonlint.com ********'
                    
            
            #convert virgin replacements file to picon name format.
            outdict = {}
            for k, v in replacements.iteritems():
                ...
                k = unicodedata.normalize('NFKD', unicode(k, 'utf_8', errors='ignore')).encode('ASCII', 'ignore')
                k = re.sub('[^a-z0-9]', '', k.replace('&', 'and').replace('+', 'plus').replace('*', 'star').lower())
                ...
                v = unicodedata.normalize('NFKD', unicode(v, 'utf_8', errors='ignore')).encode('ASCII', 'ignore')
                v = re.sub('[^a-z0-9]', '', v.replace('&', 'and').replace('+', 'plus').replace('*', 'star').lower())
                
                outdict[k] = v
                
            replacements = deepcopy(outdict)
        ...
        for line in self.lamedb:
            for channel in self.channels_all:
                ...
                if 'manual' in channel and not 'primary' in channel:
                    if channel['manual'] == True:
                        continue
                else:
                    ...
                    #add virgin replacements to list
                    if line[2] in replacements:
                        searchReplacements = [k for k, v in replacements.items() if piconname == v]
                        if line[2] in searchReplacements:
                            if line[0] not in channel['refs']:
                                channel['refs'].append(line[0])
                                break
    ...
    def createEPGDataChunks(self):
        ...
        for c in range (0, channelChunkRefsLength):
            ...
            for d in range (0, int(cfg.epgDescDays.value)):
                ...
                #timedelta used to add 24 hours for the number of days required.
                
                """
                if d == 0:
                    #todays data - 3 hours for overlap and recalculate duration of data
                    epgtime = datetime.datetime.now() - datetime.timedelta(hours=3) 
                    tomorrow = datetime.datetime.combine(datetime.date.today(), datetime.datetime.min.time()) + datetime.timedelta(days=1)
                    diff = tomorrow - epgtime
                    duration = (diff.days * 24 * 60) + (diff.seconds/60)
                else:
    ...
    def buildXMLTVSourceFile(self):
        ...
        with open(sourcepath, 'w') as f:
            ...
            if haslzma == False or cfg.compress.value == False:
